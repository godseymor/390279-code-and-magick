...
window.renderStatistics = function(ctx, names, times) {
  ...
  var randomOpacity = function() { //получение рандомной прозрачности для цвета
  ...
  for (var i = 0; i < names.length; i++) {
    if (names[i] == 'Вы') {
      ...
      ctx.fillRect(FIRST_BAR_WIDTH + (BAR_INDENT + SINGLE_BAR_WIDTH) * i, FIRST_BAR_Y, SINGLE_BAR_WIDTH, -(times[i] * BAR_CHART_HEIGHT / max));
      ctx.fillText(names[i], FIRST_BAR_WIDTH + (BAR_INDENT + SINGLE_BAR_WIDTH) * i, CLOUD_HEIGHT - INITIAL_Y);
    }
    else {
